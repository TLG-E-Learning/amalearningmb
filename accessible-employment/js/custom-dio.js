/*
Description: JS functions for MAO Acesible Employment
Author: Jabad - TLG
...
/*/
=======================================================
Loads ajax pages of the course (shell, left menu & content)
=======================================================
/*/
...
function initAjax() {
    let pageurl = getLink();
    $.ajax({
        url: "./ajax/course/course-content.html",
        success: function(result) {
            $("#init-content").html(result);
        }
    }).done(function() {
        $.ajax({
            url: "./ajax/course/left-menu.html",
            success: function(result) {
                $("#wb-sec").html(result);
        ...
        let html = "./ajax/course/s1-p1-key-features.html";
        /* if (pageurl) {
            html = pageurl;
        } */
        $.ajax({
...
function addCheckIcon(self) {
    ...
    //console.log("A check has been added")
}
...
function showDefinitionBubble() {
    $(document).on("ajaxComplete", function() {
        ...
        $(".terms").off().on('click', function() {
            ...
            var class_ = "bubble";
            var extraClass = "col-xs-6 col-sm-6 col-md-6 col-lg-8 mx-auto"
...
function loadGlossarySection() {
    $("#glossary-menu-button").click(function(e) {
        ...
        $("#glossary-container").attr("data-ajax-replace", "./ajax/course/glossary.html");
        setTimeout(function() {
...
function addSecMenuToHamburger() {
    $(window).resize(function(e) {
        if (this.innerWidth < 990) {
            $("#sec-pnl").attr("data-ajax-replace", "./ajax/course/left-menu.html");
            setTimeout(function() {
...
function showCertificate() {
    ...
    $(document).on("closed.wb-overlay", ".wb-overlay", function(evt) {
        ...
        let self = $(evt.target);
        let id = self.attr("id");
        if (id === "question-6" || id === "question-5" || id === "question-4" || id === "question-3" || id === "question-2" || id === "question-1") {

            if (parseInt(parentSection.attr("data-counter")) === parseInt(parentSection.attr("qstn-num"))) {
                $("#congrats-box").removeClass("hidden");
                document.cookie = "finalkc-status=completed";
                //console.log("data-counter==qstn-num")
            }
        }
    })
}
...
function initClearCourseProgress() {
    $("button[aria-label='Restart']").click(function() {
        localStorage.setItem("ae_courseProgress", null);
...
function saveCourseProgress() {
    ...
    if (data) {
        ...
        if (crsProgress.indexOf(pageIndex) == -1) {
            ...
            localStorage.setItem('ae_courseProgress', JSON.stringify(crsProgress));
            //console.log("data has been addded");
        }
    } else {
        localStorage.setItem('ae_courseProgress', "[" + pageIndex + "]");
        //console.log("getCourseProgress-else:" + getCourseProgress())
    }
}

function getCourseProgress() {
    if (localStorage.getItem("ae_courseProgress")) {
        let data = localStorage.getItem("ae_courseProgress");
        //console.log("retrieved data:" + data);
        return JSON.parse(data)
    } else {
        localStorage.setItem("ae_courseProgress", "[]");
        // console.log("setItem")
    }
...
function markCompletedPages() {
    ...
    if (data) {
        for (let i = 0; i < data.length; i++) {
            ...
            clearCheckIcon(selector);
            // console.log("add check on this selector:" + selector)
            setTimeout(addCheckIcon($(selector)), 1000)
        }
    }
}
...
function updateChangeLangUrl() {
    ...
    $("#main-cont").ready(function() {
        ...
        //console.log("updateChangeLang:href(" + href + ")")
    })
}

//stop video/audio that is currently playing
function stopAllMedia(obj) {
    $("video").not(obj).each(function() {
            $(this).get(0).pause()
        })
        //console.log("all media stopped")
}

//add class list to img element
function addClassList() {
    var classStr = "col-xs-10 col-sm-8 col-md-8 col-lg-8 pull-left mrgn-rght-md mrgn-bttm-lg"
        //col-md-6 pull-left col-sm-4 visible-md visible-lg mrgn-rght-md mrgn-bttm-lg
    $("#main-cont>section img").not("#alert-icon-img").each(function() {
...
function setPosition(termObj, selectorObj) {
    ...
    var bubbleOffsetLeft, bubbleOffsetTop, leftOffset, marginRight;

    leftOffset = vwportWidth - termObjOffset.left;
    marginRight = 10;
    bubble_bcr = $(".bubble").get(0).getBoundingClientRect();


    if (leftOffset > bubble_bcr.width) {

        bubbleOffsetTop = termObjOffset.top + 30;
        bubbleOffsetLeft = termObjOffset.left;

    } else {
        bubbleOffsetTop = termObjOffset.top + 30;
        bubbleOffsetLeft = vwportWidth - parseInt(bubble_bcr.width) - marginRight;

    }

    $(selectorObj).offset({
        top: bubbleOffsetTop,
        left: bubbleOffsetLeft
    })
